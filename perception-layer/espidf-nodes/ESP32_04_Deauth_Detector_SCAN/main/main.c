//perception-layer\espidf-nodes\ESP32_04_Deauth_Detector_SCAN\main\main.c

// main.c – Nodo ESP32 dinámico para escaneo multicanal de ataques Deauth
...
// Etiquetas de identificación del nodo
#define TAG "ESP32_SCANNER"
#define DEVICE_NAME "ESP32_Channel_Scanner"

// Definición de canales a escanear (excluye 1, 6 y 11)
static const uint8_t scan_channels[] = {2, 3, 4, 5, 7, 8, 9, 10, 12, 13, 14};
#define DWELL_TIME_MS 2000 // Tiempo en cada canal (2 segundos)
...
// Enviar alerta por BLE si se detecta ataque
...
// Análisis de cada paquete capturado
static void wifi_sniffer_callback(void* buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (strcmp(bssid, TARGET_BSSID) == 0) {
        ...
        if (pkt_type == 0 && pkt_subtype == 0x0C) {
            ...
            send_alert(src_mac, dst_mac, bssid, current_channel);
        }
    }
}

// BLE GAP handler (publicidad)
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t* param) {
    if (event == ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT)
        ESP_LOGI(TAG, "Advertising configurado");
}
...
// Función principal del nodo scanner
void app_main() {
    ...
    // Bucle de escaneo rotativo por canales
    int current_index = 0;
    while (1) {
        uint8_t channel = scan_channels[current_index];
        ESP_ERROR_CHECK(esp_wifi_set_channel(channel, WIFI_SECOND_CHAN_NONE));
        ESP_LOGI(TAG, "Escaneando canal %d", channel);
        vTaskDelay(pdMS_TO_TICKS(DWELL_TIME_MS));

        current_index = (current_index + 1) % (sizeof(scan_channels) / sizeof(scan_channels[0]));
    }
}
